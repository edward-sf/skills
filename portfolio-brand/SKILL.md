...
2. **Confirm before writing.** Take the brief's planned theme or recommend one; name one alternative. If the project has a styling system (Tailwind, CSS framework, component library, stylesheets), say what you found and ask how to proceed in the same question. End your turn. No file changes until the user answers.
3. **Vendor.** If `git -C <design-system> status --porcelain -- .` shows changes, stop and ask the user to commit them first. Copy `dist/tokens.css` and `base.css` (plus `dist/tokens.ts` for Expo/React Native) into `brand/` beside the UI entry point. Write `brand/VERSION`:
   ```
   design-system <sha>
   theme <name>
   date <YYYY-MM-DD>
   ```
4. **Wire.** In the entry page only: link the fonts (Public Sans 400/700/800, IBM Plex Mono 400/600 from Google Fonts), then both stylesheets after any existing stylesheet; set `data-theme="<name>"` on `<html>`, and add
   ```html
   <footer class="signature">edward-sf · <project> · $<total>/mo · <a href="https://edward-sf.dev">edward-sf.dev</a></footer>
   ```
   `<total>` is the brief's **Total incremental** (no brief: omit it). Add nothing else: no headers, nav, README or config edits.
...
If `brand/VERSION`'s sha differs from the current version, show `git -C <design-system> diff <old> HEAD -- dist/tokens.css base.css`, summarise it, ask to apply, and end your turn. On yes: re-copy, rewrite `VERSION`, update the brief's Version, commit.
...
| Theme picked with no brief and none named | Ask; write nothing |
| Editing Tailwind config or existing CSS to "integrate" | Never, unless asked in reply to step 2 |
